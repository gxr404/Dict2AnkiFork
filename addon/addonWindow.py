...
try:
  ...
  from .dummy.aqt import mw, askUser, showCritical, showInfo, tooltip, openLink
  from .dummy.noteManager import getOrCreateDeck, getDeckList, getOrCreateModel, getOrCreateModelCardTemplate, addNoteToDeck, getWordsByDeck, getNotes
...
class Windows(QDialog, mainUI.Ui_Dialog):
  ...
  def closeEvent(self, event):
    ...
    if self.updateCheckThread.isRunning():
      self.updateCheckThread.quit()
      self.updateCheckThread.wait()
  ...
  def checkUpdate(self):
    ...
    self.updateCheckWork.moveToThread(self.updateCheckThread)
    self.updateCheckWork.haveNewVersion.connect(on_haveNewVersion)
    self.updateCheckWork.finished.connect(self.updateCheckThread.quit)
  ...
  def onLogSuccess(self, cookie):
    ...
    def onAccepted():
      ...
      selectedGroups = [
        group.wordGroupListWidget.item(index).text() for index in range(group.wordGroupListWidget.count()) if
  ...
  def on_syncBtn_clicked(self):
    ...
    getOrCreateModelCardTemplate(model, 'Dict2Anki')
